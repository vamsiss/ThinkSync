...
def rate_room(request, pk):
    ...
    try:
        rating = Rating.objects.get(user=request.user, room=room)
    ...
    if request.method == "POST":
        ...
        if score:
            if rating:
                ...
                rating = Rating(user=request.user, room=room, score=int(score))
            rating.save()
            return redirect("room", pk=room.pk)

    context = {"room": room, "rating": rating.score if rating else None}
    return render(request, "base/rate_room.html", context)
